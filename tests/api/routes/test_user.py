...
class TestGetUser:
    ...
    async def test_get_user_successfully(
        self, get_user_mock: AsyncMock, client: TestClient, user_db_fixture: User
    ):
    ...
    @pytest.mark.asyncio()
    @patch(f"{user_service_path}.get_user")
    async def test_get_user_does_not_exist_error(
        self, get_user_mock: AsyncMock, client: TestClient, user_db_fixture: User
    ):
        ...
        assert result.json() == {"detail": "user doesn't exist"}

        get_user_mock.assert_awaited_with(1)

    @pytest.mark.asyncio()
    async def test_get_user_wrong_input_param_type(
        self, client: TestClient, user_db_fixture: User
    ):
    ...
    @pytest.mark.asyncio()
    async def test_get_user_skip_required_param(
        self, client: TestClient, user_db_fixture: User
    ):
...
class TestCreateUser:
    ...
    async def test_create_user_user_already_exist_error(
        self, create_user_mock: AsyncMock, client: TestClient, user_db_fixture: User
    ):
    ...
    async def test_create_user_skip_required_param(
        ...
        result = client.post(
            f"{create_user_url}/",
            json={"login": "test_user", "password_wrong": "test_password"},
        )
        assert result.status_code == 422
        assert result.json() == {
            "detail": [
...
class TestUpdateUser:
    ...
    async def test_update_user_skip_required_param(
        self, client: TestClient, user_db_fixture: User, jwt_for_user_fixture: str
    ):
        result = client.patch(
            f"{update_user_url}/",
            headers={"Authorization": f"Bearer {jwt_for_user_fixture}"},
